...
(function(){
    ...
    .controller('editTamanhoController', ['$scope', '$http', '$routeParams', '$location', 'tamanhoService', '$window', 'utilsService', function($scope, $http, $routeParams, $location, tamanhoService, $window, utilsService){

        $scope.isUpdate = $routeParams.codTamanho !== undefined;
        ...
        $scope.save = function(){
            if ($scope.isUpdate){
                ...
                if ($scope.tamanho.desTamanho === ""){
                    delete($scope.tamanho.desTamanho);
                }
                // TODO: the following line will remain just until datepicker strategy is up and running
                delete($scope.tamanho.datCadastro);
                $http.put(utilsService.apiProtectedUrl + '/tamanhos/' + $routeParams.codTamanho, $scope.tamanho).then(function(res){
                    if (res.status == 204){
                        $window.history.back();
                    } else {
                ...
                $http.post(utilsService.apiProtectedUrl + '/tamanhos', $scope.tamanho).then(function(res){
                    if (res.status == 201){
                        $window.history.back();
                    } else {
                        alert(res.status + JSON.stringify(res.data));   
        ...
        if (!$scope.isUpdate){
            $scope.tamanho.indAtivo = 'S';
            $scope.tamanho.codEstabelecimento = $routeParams.codEstabelecimento;
        } else {
            for (var i in $scope.tamanhos){
                if ($scope.tamanhos[i].codTamanho == $routeParams.codTamanho){
