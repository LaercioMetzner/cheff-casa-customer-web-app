...
(function(){
    ...
    .controller('listEstabelecimentosController', ['$scope', '$http', '$location', 'estabelecimentoService', 'utilsService', 'modalService', function($scope, $http, $location, estabelecimentoService, utilsService, modalService){

    $scope.editEstabelecimento = function(estabelecimento){
            $location.url('/editEstabelecimento/' + estabelecimento.codEstabelecimento);
    }
    $scope.newEstabelecimento = function(){
            $location.url('/editEstabelecimento');
    }

    $http.get(utilsService.apiProtectedUrl + '/estabelecimentos').then(function(res){
        estabelecimentoService.estabelecimentos = res.data;
