...
function MascaraCPFouCNPJ(cpf){
        if(mascaraInteiro(cpf)==false){
                event.returnValue = false;
        }   
        if (cpf.value.toString().length >= 14){
...
//adiciona mascara ao Whatsapp
function MascaraWhatsapp(tel){  
        if(mascaraInteiro(tel)==false){
                event.returnValue = false;
        }       
        if (tel.value.toString().length >= 16){
            return formataCampo(tel, '+00 00 00000-0000', event);
        } else {
